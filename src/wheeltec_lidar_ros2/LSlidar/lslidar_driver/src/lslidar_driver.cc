...
namespace lslidar_driver
{
	...
	bool LslidarDriver::loadParameters()
	{
		...
		this->declare_parameter<std::vector<double>>("scan_crop_min", std::vector<double>{});
		this->declare_parameter<std::vector<double>>("scan_crop_max", std::vector<double>{});
		...
		this->get_parameter("scan_crop_min", scan_crop_min);
		this->get_parameter("scan_crop_max", scan_crop_max);
		
		// 验证多角度屏蔽参数
		if (truncated_mode_ == 1) {
			if (scan_crop_min.size() != scan_crop_max.size()) {
				RCLCPP_ERROR(this->get_logger(), "scan_crop_min and scan_crop_max must have the same size!");
				truncated_mode_ = 0;
			} else if (scan_crop_min.size() == 0) {
				RCLCPP_WARN(this->get_logger(), "truncated_mode_ is enabled but no crop angles specified, disabling multi-angle cropping");
				truncated_mode_ = 0;
			} else {
				RCLCPP_INFO(this->get_logger(), "Multi-angle cropping enabled with %zu angle ranges", scan_crop_min.size());
				for (size_t i = 0; i < scan_crop_min.size(); i++) {
					RCLCPP_INFO(this->get_logger(), "Crop range %zu: [%.1f°, %.1f°]", i, scan_crop_min[i], scan_crop_max[i]);
				}
			}
		}
