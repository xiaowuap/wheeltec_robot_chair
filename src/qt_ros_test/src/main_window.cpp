...
{
    if(ui.label_image->width()*0.75<=ui.label_image->height())
        ui.label_image->setPixmap(QPixmap::fromImage(im).scaled(ui.label_image->width(), ui.label_image->width()*0.75));
    else
        ui.label_image->setPixmap(QPixmap::fromImage(im).scaled(ui.label_image->height()*1.33, ui.label_image->height()));
}
//电量显示
...
void qt_ros_test::MainWindow::on_pushButton_2dmap_clicked(bool checked)
{
    ...
    else
    {
        closeroslaunch();
        QProcess* rviz_cmd =new QProcess(this);
        rviz_cmd->startDetached("killall -2 rviz");
        _cmd->waitForFinished(10000);
