...
void GNSSPoser::callbackNavSatFix(const sensor_msgs::NavSatFix::ConstPtr & nav_sat_fix_msg_ptr)
{
  ...
  geo_pos_conv geo;
  geo.set_plane(7);
  geo.llh_to_xyz(nav_sat_fix_msg_ptr->latitude, nav_sat_fix_msg_ptr->longitude, nav_sat_fix_msg_ptr->altitude);
  ...
  const ros::Time & stamp)
{
  ...
  try {
    ...
    *transform_stamped_ptr =
      tf2_buffer_.lookupTransform(target_frame, source_frame, ros::Time(0), ros::Duration(0.5));
  } catch (tf2::TransformException & ex) {
    ROS_WARN_STREAM_THROTTLE(1, ex.what());
    ROS_WARN_STREAM_THROTTLE(
      1, "Please publish TF " << target_frame.c_str() << " to " << source_frame.c_str());
