...
GNSSStat NavSatFix2ECEF(const sensor_msgs::NavSatFix & nav_sat_fix_msg)
{
  GNSSStat ecef_ori;
  GNSSStat ecef;
  GeographicLib::Geocentric geoConverter;
  ecef.coordinate_system = CoordinateSystem::ECEF;
  double ecef_ori_x, ecef_ori_y, ecef_ori_z;
  try {
    geoConverter.Forward(
      nav_sat_fix_msg.latitude, nav_sat_fix_msg.longitude,nav_sat_fix_msg.altitude, ecef_ori_x, ecef_ori_y, ecef_ori_z);
      if(!(ecef_ori_x==0&&ecef_ori_y==0&ecef_ori_z==0))
      {
          ecef.x=ecef_ori_z;
          ecef.y=ecef_ori_z;
          ecef.z=ecef_ori_z;
    ...
    ROS_ERROR_STREAM("Failed to convert from LLH to ECEF" << err.what());
  }
  return ecef;
