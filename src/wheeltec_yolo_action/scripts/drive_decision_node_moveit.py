		...
		if control_flag == 1:
			if currentState == 0:		#init    
				...
				if sign_d_flag == 0 and move_flag == 0 and count2 != -1:
					...
					ctrl_pub.publish(car_stop)
					if signd_x > 320 :
						erro_x = signd_x - ((right_sign_xmin +right_sign_xmax)/2)
						erro_y = signd_y - ((right_sign_ymin +right_sign_ymax)/2)
					else :
						erro_x = signd_x - ((left_sign_xmin +left_sign_xmax)/2)
						erro_y = signd_y - ((left_sign_ymin +left_sign_ymax)/2)
					d_erro_x = erro_x - last_erro_x
					d_erro_y = erro_y - last_erro_y
					carmove = Twist()
					carmove.linear.x = -float(erro_y)*0.0003 - float(d_erro_y)*0.0
